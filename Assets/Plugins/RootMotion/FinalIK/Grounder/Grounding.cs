...
namespace RootMotion.FinalIK {
	...
	public partial class Grounding {
		...
		/// <summary>
		/// The acceleration of prediction.
		/// </summary>
		public float predictionAcceleration = 5f;
		/// <summary>
		...
		/// <summary>
		/// Max Foot Rotation Angle", "Max angular offset from the foot's rotation.
		/// </summary>
		...
		/// <summary>
		/// Used for smoothing out vertical pelvis movement
		/// </summary>
		...
		public bool rootGrounded {
			get {
				if (quality == Quality.Fastest) return true;
		...
		public void Update() {
			...
			footRadius = Mathf.Clamp(footRadius, 0f, maxStep);

			// Root hit
			rootHit = GetRootHit();

			float maxStepHeightFromGround = Mathf.NegativeInfinity;
			isGrounded = false;
			...
			foreach (Leg leg in legs) {
				...
				if (leg.IKOffset > maxStepHeightFromGround) maxStepHeightFromGround = leg.IKOffset;
				if (leg.isGrounded) isGrounded = true;
			}
			
			// Precess pelvis
			pelvis.Process(maxStepHeightFromGround, isGrounded);
		}
		...
		// Raycasts or sphereCasts to find the root ground point
		private RaycastHit GetRootHit() {
			RaycastHit h = new RaycastHit();
			Vector3 _up = up;
			h.point = root.position - _up * maxStep * 10f;
			h.distance = maxStep * 11f;
			
			if (quality != Quality.Best) Physics.Raycast(root.position + _up * maxStep, -_up, out h, maxStep * 11f, layers);
			else Physics.SphereCast(root.position + _up * maxStep, rootSphereCastRadius, -up, out h, maxStep * 11f, layers);
