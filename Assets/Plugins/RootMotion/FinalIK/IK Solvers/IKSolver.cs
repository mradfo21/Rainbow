	...
	public abstract class IKSolver {
		...
		public class Bone: Point {
			...
			public void Swing(Vector3 swingTarget, float weight = 1f) {
				if (weight <= 0f) return; 
				if (weight >= 1f) transform.rotation = Quaternion.FromToRotation(transform.rotation * axis, swingTarget - transform.position) * transform.rotation;
				transform.rotation = Quaternion.Slerp(Quaternion.identity, Quaternion.FromToRotation(transform.rotation * axis, swingTarget - transform.position), weight) * transform.rotation;
			}
